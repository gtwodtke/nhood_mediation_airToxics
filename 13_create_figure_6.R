...
################################################
##                                            ##
## PROGRAM NAME: 13_create_figure_6           ##
## AUTHOR: GW                                 ##
## DESCRIPTION:                               ##
##                                            ##
##  create plots of chem SHAP importance for  ##
##  test scores                               ##
##                                            ##
################################################
################################################
...
##### COMPUTE M->Y SHAP IMPORTANCE #####
miest.rd.w1<-miest.mt.w1<-matrix(data=NA,nrow=length(vars.ntxw1),ncol=nmi)
...
for (i in 1:nmi) {
	...
	shap.chem.rd<-shap.rd[vars.ntxw1]
	shap.chem.mt<-shap.mt[vars.ntxw1]
	shap.chem.imp.rd<-data.frame(label=chem.label, importance=apply(shap.chem.rd,MARGIN=2,FUN=function(x) mean(abs(x))))
	shap.chem.imp.mt<-data.frame(label=chem.label, importance=apply(shap.chem.mt,MARGIN=2,FUN=function(x) mean(abs(x))))
	miest.rd.w1[,i]<-shap.chem.imp.rd[,"importance"]
	miest.mt.w1[,i]<-shap.chem.imp.mt[,"importance"]
	}

### COMBINE MI ESTIMATES ###
est.rd.w1<-est.mt.w1<-matrix(data=NA,nrow=length(vars.ntxw1),ncol=2)

for (i in 1:length(vars.ntxw1)) { 

	est.rd.w1[i,1]<-est.mt.w1[i,1]<-chem.label[i]
	est.rd.w1[i,2]<-round(mean(miest.rd.w1[i,]),digits=4)
	est.mt.w1[i,2]<-round(mean(miest.mt.w1[i,]),digits=4)
	}

### PRINT RESULTS ###
sink("C:\\Users\\wodtke\\Desktop\\projects\\nhood_mediation_toxins\\programs\\_LOGS\\13_create_figure_6_log.txt")
...
rownames(output.rd.w1)<-rownames(output.mt.w1)<-vars.ntxw1
...
tiff("C:\\Users\\wodtke\\Desktop\\projects\\nhood_mediation_toxins\\figures\\figure_6.tiff",
	height=9,
	width=9,
	units='in',
	res=600)
