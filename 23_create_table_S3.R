...
################################################
##                                            ##
## PROGRAM NAME: 23_create_table_S3           ##
## AUTHOR: GW                                 ##
## DESCRIPTION:                               ##
##                                            ##
##  create table of nh effect estimates       ##
##  based on nhdadvg scale                    ## 
##                                            ##
################################################
################################################
...
eclsb<-as.data.frame(eclsb[,c(vars.meta,vars.base,vars.y,vars.covw1,vars.ntxw1)])
eclsb.mi.vars<-eclsb[,-which(names(eclsb) %in% c(vars.meta))]

##### IMPUTE MISSING DATA #####
eclsb.imp<-imp.rfnode.cond(eclsb.mi.vars,num.imp=nmi,seed=8675309,num.threads=n.cores)

eclsb$minum<-0
for (i in 1:nmi) {
	mi<-complete(eclsb.imp,action=i)
...
eclsb.mi<-as.data.frame(eclsb[which(eclsb$minum!=0),])
eclsb.mi<-eclsb.mi[order(eclsb.mi$caseid,eclsb.mi$minum),]
rm(list=c("eclsb","eclsb.imp"))
...
astar<-quantile(eclsb.mi$nhdadvg01,probs=0.9,na.rm=T)
a<-quantile(eclsb.mi$nhdadvg01,probs=0.1,na.rm=T)
...
for (i in 1:nmi) {
	...
	eclsb.tune.imp$nhdadvg01<-astar	
	...
	boot.est.w1<-foreach(h=1:nboot, .combine=cbind) %dopar% {
		...
		boot.c.a.m.imp$nhdadvg01<-astar	
...
### PRINT RESULTS ###
sink("C:\\Users\\wodtke\\Desktop\\projects\\nhood_mediation_toxins\\programs\\_LOGS\\23_create_table_S3_log.txt")
